        ...
        <main class="flex-grow flex items-center justify-center px-4 relative z-10 py-24">
            ...
            <div class="w-full max-w-md">
                ...
                <div class="text-center mb-8">
                    <h1 class="text-3xl font-bold text-white mb-2 tracking-tight">Bem-vindo de volta</h1>
                    <p class="text-slate-400 text-sm">Acesse sua conta para continuar investindo.</p>
                </div>
                ...
                <div class="bg-[#0F172A]/90 backdrop-blur-md border border-slate-800 rounded-2xl shadow-2xl p-6 sm:p-8">
                    ...
                    <form @submit.prevent="handleLogin" class="space-y-5">
                        ...
                        <div>
                            ...
                            <div class="relative input-fintech rounded-lg flex items-center">
                                ...
                                <input v-model="loginData.senha" :type="showPassword ? 'text' : 'password'"
                                    id="password"
                                    class="w-full bg-transparent border-none text-white px-3 py-3.5 focus:outline-none placeholder-slate-600 text-sm"
                        ...
                        <div class="flex items-center">
                            ...
                            <label for="remember"
                                class="ml-2 block text-sm text-slate-400 cursor-pointer select-none">Lembrar meus
                                dados</label>
                        </div>

                        <div v-if="errorMessage"
                            class="text-red-500 text-sm text-center font-medium bg-red-500/10 py-2 rounded">
                            {{ errorMessage }}
                        </div>
                    ...
                    <div class="mt-8 mb-6 relative">
                        ...
                        <div class="relative flex justify-center text-sm">
                            <span
                                class="px-3 bg-[#0F172A] text-slate-500 text-xs uppercase tracking-wider font-semibold">Ou
                                acesse
                                com</span>
                        </div>
                    </div>
                ...
                <p class="mt-8 text-center text-sm text-slate-400">
                    ...
                    <router-link to="/registro"
                        class="font-semibold text-prime-500 hover:text-prime-400 transition-colors">Abra sua
                        conta grátis</router-link>
                </p>
...
// VARIÁVEL E FUNÇÃO DA SENHA ADICIONADAS AQUI:
...
// Função auxiliar para forçar a renderização dos ícones do Lucide
const renderIcons = () => {
    if (window.lucide) {
...
const handleLogin = async () => {
    ...
    try {
        ...
        // Atualiza os ícones após tirar o loading do botão
        nextTick(() => {
            renderIcons()
        })
    }
...
// Sempre que ocorrer uma mudança reativa (como clicar no botão de ver senha), o Vue re-renderiza o DOM. 
// O onUpdated garante que o ícone novo injetado pelo v-if/v-else seja lido pelo Lucide.
onUpdated(() => {
    nextTick(() => {
        renderIcons()
    })
